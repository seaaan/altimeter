...
boolean Adafruit_MPL3115A2::isAltitudeReady() {
  return read8(MPL3115A2_REGISTER_STATUS) & MPL3115A2_REGISTER_STATUS_PDR;
}

float Adafruit_MPL3115A2::readAltitude() {
  Wire.beginTransmission(MPL3115A2_ADDRESS); // start transmission to device 
  Wire.write(MPL3115A2_REGISTER_PRESSURE_MSB); 
  Wire.endTransmission(false); // end transmission
  
  Wire.requestFrom((uint8_t)MPL3115A2_ADDRESS, (uint8_t)3);// send data n-bytes read
  int32_t alt = Wire.read(); // receive DATA
  alt <<= 8;
...
float Adafruit_MPL3115A2::getAltitude() {
  startReadingAltitude();  

  while (!isAltitudeReady()) {
    delay(10);
  }
